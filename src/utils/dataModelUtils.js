...
export function normalizeCrmData(input) {
  ...
  const properties = (source.properties || []).map((property) => {
    ...
    return {
      ...
      title: property.title || `${property.propertyType || 'property'} in ${property.location || 'Egypt'}`,
      ...
      marketingStatus: property.marketingStatus || (property.agreementType === 'exclusive' ? 'sponsored' : 'listed'),
      photosCount: property.photosCount ?? 0,
      documentsCount: property.documentsCount ?? 0,
      attachmentsCount: property.attachmentsCount ?? property.documentsCount ?? 0,
      lastActivityAt: property.lastActivityAt || property.updatedAt || now,
      nextFollowUpAt: property.nextFollowUpAt || null,
